...
describe ('Movie info block', () => {
  ...
  it('Should have movie poster with image, alt description ans source path', () => {
    ...
    movie__poster.forEach(movie__poster => {
      cy.check_poster(movie__poster.number, 'alt', movie__poster.alt); //селектор захаржкожен в commands поскольку имеет переменную часть, код теста и комманды при увеличении кол-ва фильмов меняться не будет
